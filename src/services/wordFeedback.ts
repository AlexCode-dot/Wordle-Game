export default function generateWordFeedback(guessWord: string, correctWord: string) {
  ...
  if (guessArray.length === correctArray.length) {
    const guessFeedback = guessArray.map((item: string) => ({
      letter: item,
      result: '',
    }))

    const letterFrequency = correctArray.reduce((count: { [key: string]: number }, letter: string) => {
